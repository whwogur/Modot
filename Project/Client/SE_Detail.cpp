...
void SE_Detail::Update()
{
	...
	ImGui::SameLine();
	ClientStatic::ToggleButton("##PrecisionMode", &PrecisionMode);
	ImGui::SameLine();
	...
	if (PrecisionMode)
	{
		...
		if (m_LeftTop.x >= 0 && m_Slice.x >= 0)
		{
			...
			ImGui::NewLine();
			ImGui::SameLine(50);
			ImGui::Image((ImTextureID)m_AtlasTex.Get()->GetSRV().Get(), { 250, 250 }, { vLT[0] / width, vLT[1] / height }, { vRB[0] / width, vRB[1] / height }, { 1, 1, 1, 1 }, HEADER_2);
		}
	}
	else
	{
