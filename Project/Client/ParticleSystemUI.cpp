	...
	if (!Collapsed())
	{
		...
		if (curObj != nullptr)
		{
			...
			if (curParticleSys != nullptr)
			{
				...
				ClientStatic::ToggleButton("##sharedSpawnModule", &mod.Module[(UINT)PARTICLE_MODULE::SPAWN]);
				...
				ClientStatic::ToggleButton("##sharedBurstModule", &mod.Module[(UINT)PARTICLE_MODULE::SPAWN_BURST]);
				...
				ClientStatic::ToggleButton("##sharedScaleModule", &mod.Module[(UINT)PARTICLE_MODULE::SCALE]);
				ImGui::InputFloat("Start Scale", &mod.StartScale);
				ImGui::InputFloat("End Scale", &mod.EndScale);

				ImGui::SeparatorText(u8"Velocity 모듈");
				ClientStatic::ToggleButton("##sharedVelocityModule", &mod.Module[(UINT)PARTICLE_MODULE::ADD_VELOCITY]);
				...
				ClientStatic::ToggleButton("##sharedDragModule", &mod.Module[(UINT)PARTICLE_MODULE::DRAG]);
				...
				ClientStatic::ToggleButton("##sharedNoiseModule", &mod.Module[(UINT)PARTICLE_MODULE::NOISE_FORCE]);
				...
				ClientStatic::ToggleButton("##sharedRenderModule", &mod.Module[(UINT)PARTICLE_MODULE::RENDER]);
				...
				ClientStatic::ToggleButton("##sharedRotModule", &mod.Module[(UINT)PARTICLE_MODULE::ORBIT]);
				ImGui::InputFloat("Max Rotation", &mod.MaxRotationSpeed);
				ImGui::SeparatorText(u8"Gyrate");
				ClientStatic::ToggleButton("##sharedGyrate", &mod.Gyrate);
