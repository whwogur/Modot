...
namespace Modot {
    ...
    IMGUI_API const ImVec4& GetStyleModotSequencerColorVec4(ModotSequencerCol idx);
    IMGUI_API ModotSequencerStyle& GetModotSequencerStyle();

    IMGUI_API void PushModotSequencerStyleColor(ModotSequencerCol idx, ImU32 col);
    IMGUI_API void PushModotSequencerStyleColor(ModotSequencerCol idx, const ImVec4& col);
    IMGUI_API void PopModotSequencerStyleColor(int count = 1);

    IMGUI_API bool BeginModotSequencer(const char* id, FrameIndexType * frame, FrameIndexType * startFrame, FrameIndexType * endFrame,const ImVec2& size = ImVec2(0, 0),ModotSequencerFlags flags = ModotSequencerFlags_None);
    IMGUI_API void EndModotSequencer(); //Call only when BeginModotSequencer() returns true!!
    ...
    IMGUI_API bool BeginModotTimelineEx(const char* label, bool * open = nullptr, ModotTimelineFlags flags = ModotTimelineFlags_None);
    ...
    IMGUI_API void ModotClearSelection(); // Clears selection
    IMGUI_API bool ModotIsSelecting(); // Are we currently selecting?
    IMGUI_API bool ModotHasSelection(); // Is anything selected?
    IMGUI_API bool ModotIsDraggingSelection(); // Are we dragging selection?
    IMGUI_API bool ModotCanDeleteSelection(); // Can selection deletion be done?
    ...
    IMGUI_API bool IsModotTimelineSelected(ModotTimelineIsSelectedFlags flags = ModotTimelineIsSelectedFlags_None);

#ifdef __cplusplus
    // C++ helper
    IMGUI_API bool BeginModotTimeline(const char* label,std::vector<int32_t> & keyframes ,bool * open = nullptr, ModotTimelineFlags flags = ModotTimelineFlags_None);
#endif
}
